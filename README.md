...
### Colors and fonts
   * Created colormap class analagous to `LinearSegmentedColormap` called `PerceptuallyUniformColormap`, which interpolates through hue, saturation, and luminance space (with hues allowed to vary circularly). Choose from either of 4 HSV-like colorspaces: classic HSV, perceptually uniform HCL, and HSLuv and HPLuv which are forms of HCL adapted for convenient use with colormaps.
   * Create perceptually uniformly varying colormaps on-the-fly by passing a dictionary to any plotting function that accepts the `cmap`keyword argument -- for example, `ax,contourf(..., cmap=dict(h=hues, l=lums, s=sats))`. The arguments can be lists of numbers or **color strings**, in which case the corresponding channel value (hue, saturation, or luminance) for that color will be looked up and applied.
   * Create single-hue colormaps on-the-fly by passing a string that looks like `cmap='name_[mode]'` or `cmap='name_d'`, where `name` is any registered color string (the corresponding hue will be looked up). The colormap mode can be either of `l` (vary color from its current lightness to near-white), `d` (vary color/chroma from its current lightness/saturation to pure gray), `w` (as in `l`, but use pure white), and `b` (as in `d`, but use pure black). 
   * The definition of "colormaps" and "color cyclers" is now fluid. Specify `plot.rc(cycle='name')` where name is the name of any `LinearSegmentedColormap` or `ListedColormap` to generate a color cycler from that colormap. 
   ...
   * Cycles are all defined as `ListedColormaps`, and we have added several new ones (default is `'colorblind'`).
   * Added many new colors, including XKCD colors, crayon colors, and Open Color web-design color palette.
   * Added new color "cycles" (i.e. the automatic color order when drawing multiple lines), which can be set with `plot.rc(cycle='name')` or by passing `cycle='name'` to any command that plots lines/patches (`plot`, `bar`, etc.).
