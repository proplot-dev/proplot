...
"""
from . import utils
from . import ncutils as nc
from . import constants as c
import os
import numpy as np
__all__ = [
    'graticule', 'downsample',
    'lonfix', 'latfix', 'geopad',
    'geomean', 'geoweights', # geographic mean, weightings
    'haversine',
        ]
...
def lonfix(lon, data=None, lon_0=-180, roll=True, return_roll=False):
    ...
    # Roll back, if roll is False; just wanted to adjust lon values; needed to roll before
...
def geopad(lon, lat, data, nlon=1, nlat=0):
    ...
    NOTE, SHOULD ALSO MAYBE MAKE SOMETHING THAT PADS FOR CONTOURING.
    """
    # Get padded array
    if nlon>0:
    ...
    if nlat>0:
        ...
        data_append = np.roll(np.flip(data, axis=1), shift=int(data.shape[1]/2), axis=1)
        data = np.concatenate((
...
def landfracs(lonb, latb=None, lon_0=-180):
    """
    Retrieve land fractions on the provided graticule.
    Input options:
        1) landfracs(latb, lonb) -- include graticule vectors.
        2) landfracs(latres, lonres) -- use specified latitude, longitude resolution.
        3) landfracs(res) -- use same resolution in latitude and longitude.
    """
    file = os.environ['HOME'] + '/landfracs.nc'
    F, f = nc.ncload(file, 'data', graticule=True, verbose=False, squeeze=True) # want to return graticule
    if latb is None:
        latb = lonb
    # Latitudes
    try:
        ...
        lonb = utils.arange(lon_0, lon_0+360, lonb) # like np.arange, but includes endpoints
    else:
    ...
    try:
        ...
        latb = utils.arange(-90, 90, latb) # like np.arange, but includes endpoints
    else:
        if type(latb) is not np.ndarray: latb = np.array(latb)
    # And downsample
    return downsample(f.lonb, f.latb, F, lonb, latb)

def geoweights(lonb, latb):
    """
    Gets approximate area of grid cells.
    """
    thetab, phib = lonb*np.pi/180, latb*np.pi/180
    theta, phi = (thetab[1:]+thetab[:-1])/2, (phib[1:]+phib[:-1])/2, 
    return ((c.a**2) # area component
            * (np.abs(np.cos(phi))*np.diff(phib))[np.newaxis,...] # cosine latitude, cosine width
            * np.diff(thetab)[...,np.newaxis]) # longitude width

def geomean(lonb, latb, data, box=(None,)*4, landfracs=None, mode=None, weights=1, keepdims=False):
    """
    ...
    """
    # First get lon, lat centers and box widths for weighting
    # ...these operations should be super fast; probably shouldn't worry if calling
    # ...geomean a bunch of times; less than 10us each
    delta_lon = (lonb[1:]-lonb[:-1])[...,np.newaxis]
    delta_lat = (latb[1:]-latb[:-1])[np.newaxis,...]
    lon = (lonb[1:]+lonb[:-1])/2
    lat = (latb[1:]+latb[:-1])/2
    # Get zone
    ...
    if box[0] is not None:
        lune = lune & (lonb[:-1]>=box[0])[...,np.newaxis]
    if box[2] is not None:
        lune = lune & (lonb[1:]<=box[2])[...,np.newaxis]
    # Get area weightings
    cos = np.abs(np.cos(np.deg2rad(lat[np.newaxis,...])))
    # Get total weights
    w = delta_lat*delta_lon*cos*zone*lune*weights
    for s in data.shape[2:]:
        w = w[...,np.newaxis]
    w = np.tile(w, (1,1,*data.shape[2:])) # has to be tiled
    # And apply, allowing for different data types; will squeeze axes 0 and 1
    if type(data) is np.ma.MaskedArray:
        ave = np.ma.average(data, weights=w, axis=(0,1))
    elif type(data) is np.ndarray: # about the same speed, actually
        f = np.isnan(data)
        data[f] = 0 # this assignment is pretty fast
        ave = np.average(data, weights=w*(~f), axis=(0,1))
    else:
