...
* The ``abc``, ``title``, and ``tick`` categories control
  the subplot a-b-c labels, titles, and tick labels.
...
* The ``formatter`` category is a shorthand for matplotlib's ``axes.formatter``
  and includes new settings that control the tick labels generated by
  `~proplot.ticker.AutoFormatter`.
* The ``gridminor`` category optionally controls minor gridlines separately
  from major gridlines.
* The matplotlib ``image`` category includes new settings that control the
  colormap levels generated by `~proplot.colors.DiscreteNorm`.
* The matplotlib ``grid`` category includes new settings that control
  the meridian and parallel gridlines managed by `~proplot.axes.GeoAxes`.
