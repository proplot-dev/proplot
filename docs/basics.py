...
# `~proplot.ui.subplots` commands. `~proplot.ui.subplots` creates a figure and a grid
# of subplots all at once, while `~proplot.ui.figure` creates an empty figure that
# can be subsequently filled with `proplot.figure.Figure.add_subplot` or
# `proplot.figure.Figure.add_subplots` (or their shorthands
# `~proplot.figure.Figure.subplot` or `~proplot.figure.Figure.subplots`).
# Internally, as in matplotlib, `~proplot.ui.subplots` simply calls
# `~proplot.ui.figure` followed by `proplot.figure.Figure.subplots`.
#
# The `~proplot.figure.Figure.subplots` and `~proplot.figure.Figure.add_subplots`
...
#
# The `~proplot.figure.Figure.subplot` and `~proplot.figure.Figure.add_subplot`
# commands can be used as follows:
#
...
# with `os.path.expanduser`. In the below examples, we create a few simple
# figures with a variety of methods. These should be generally familiar
# to existing matplotlib users.
...
#
#    ProPlot also changes the default :rcraw:`savefig.format` from
#    PNG to PDF for the following reasons:
#
