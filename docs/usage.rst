...
ProPlot is an *object-oriented* matplotlib wrapper. That means that
ProPlot's features are largely a **superset** of matplotlib's features.
...
The *wrapper* part means that ProPlot's features are implemented with subclasses of the `~matplotlib.figure.Figure` and `~matplotlib.axes.Axes` classes.

If you tend to use `~matplotlib.pyplot` and are not familiar with figure and axes *objects*, check out `this guide from the matplotlib documentation <https://matplotlib.org/api/api_overview.html#the-pyplot-api>`__. Working with objects directly tends to be more clear and concise than `~matplotlib.pyplot`, makes life much easier when working with multiple figures or axes, and is certainly more "`pythonic <https://www.python.org/dev/peps/pep-0020/>`__". Therefore, although some ProPlot features may still work, ProPlot does not officially support the `~matplotlib.pyplot` API.
